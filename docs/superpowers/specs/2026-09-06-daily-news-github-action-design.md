...
- Call `GET https://newsapi.org/v2/top-headlines?country=us&category={category}&pageSize=5` with header `X-Api-Key: $NEWS_API_KEY` (key read from the `NEWS_API_KEY` env var, sourced from the GitHub Actions secret of the same name — already added to the repo).
- From each returned article, keep exactly the fields already present in `daily-news.json`: `title`, `description`, `url`, `source` (flattened from `source.name`), `publishedAt`, `image` (mapped from NewsAPI's `urlToImage`).
...
- If any of the three API calls fails (non-200 response, network error, malformed JSON), the script prints the error to stderr and exits with a non-zero status **before** writing any file.
...
- A category returning fewer than 5 articles (as `business` did historically) is not an error — write whatever NewsAPI returns for that category, unless it's zero articles, which is treated as a failure for that category (likely signals a bad API response rather than a genuinely quiet news day for a top-headlines category).

### Commit step

After a successful fetch, the workflow:
1. Configures git identity as `github-actions[bot]`.
2. `git add data/daily-news.json`.
3. Commits only if there's a diff (`git diff --cached --quiet || git commit -m "Update daily news"`) — avoids empty commits when content is unchanged.
4. Pushes using the workflow's built-in `GITHUB_TOKEN` (scoped to this repo, expires with the run) — no third-party commit action, no PAT.
